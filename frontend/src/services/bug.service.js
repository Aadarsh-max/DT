...
export async function updateBug(id, user, data) {
  ...
  if (assigneeId !== undefined) {
    if (assigneeId !== null) {
      const people = await projectPeople(bug.projectId);
      if (!people.includes(assigneeId)) {
        throw ApiError.badRequest('A bug can only be assigned to the project owner or a team member');
      }
    }
    patch.assigneeId = assigneeId;
  }

  if (Object.keys(patch).length) await prisma.bug.update({ where: { id }, data: patch });

  if (assigneeId && assigneeId !== bug.assigneeId && assigneeId !== user.id) {
    await notifyUsers([assigneeId], {
      title: `${user.name} assigned you ${bugCode(bug.number)}`,
      message: bug.title,
      link: `/bugs/${id}`,
    });
  }
