...
const MAX_OPTIONS = [5, 10, 20, 30, 50].map((n) => ({ value: String(n), label: `Up to ${n} test cases` }));
...
export default function NewRunModal({ open, onClose, project, onStarted }) {
  ...
  return (
    <Modal open={open} onClose={() => !busy && onClose()} title="New test run" className="sm:max-w-xl">
      <div className="space-y-5">
              ...
              return (
                ...
                >
                  ...
                  <span className="min-w-0 text-sm">
                    <span className="block font-semibold text-ink">{TEST_TYPE_LABEL[t]}</span>
        ...
        <div>
          <label className="mb-1.5 block text-sm font-medium text-ink">How many to run</label>
          <Select className="h-10" options={MAX_OPTIONS} value={maxCases} onChange={(e) => setMaxCases(e.target.value)} />
          <p className="mt-1.5 text-xs text-muted">Highest priority cases run first.</p>
        </div>
        ...
        {hasUi && (
          <label className="flex cursor-pointer items-start gap-2.5 text-sm">
            ...
            />
            <span>
              <span className="block font-medium text-ink">Show the browser window</span>
              <span className="block text-xs text-muted">
        ...
        <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          ...
          <Button icon={Play} loading={busy} disabled={!canSubmit} onClick={submit}>
            Start run
          </Button>
        </div>
      </div>
