...
import { ChevronLeft, ChevronRight, ClipboardList, FolderOpen, Search, Sparkles, Trash2, X } from 'lucide-react';
...
import { PRIORITY_OPTIONS, TEST_TYPES, TEST_TYPE_LABEL } from '../utils/constants';
...
function GenerationBanner({ snapshot, onDismiss }) {
  ...
  if (state === 'completed') message = `Done. ${snapshot.result?.created ?? 0} new test cases added.`;
  if (state === 'failed') message = snapshot.error || 'Generation failed';
  if (state === 'unknown') message = snapshot?.error || 'This job is no longer available.';
    ...
    <Card>
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2.5">
          {finished ? (
            <Sparkles className={bad ? 'size-5 text-danger' : 'size-5 text-success'} />
          ) : (
            <Spinner className="size-4 text-primary" />
          )}
          <div className="min-w-0">
            <p className="text-sm font-semibold text-ink">AI test generation</p>
            <p className={`text-sm ${bad ? 'text-danger' : 'text-muted'}`}>{message}</p>
          </div>
        </div>
        {finished && (
          <button
...
export default function TestCases() {
  ...
  const { snapshot } = useRunProgress({
    ...
    streamPath: jobId ? `/projects/${projectId}/testcases/generate/${jobId}/events` : null,
    statusPath: jobId ? `/projects/${projectId}/testcases/generate/${jobId}` : null,
    onFinish: (snap) => {
      if (snap.state === 'completed') toast.success(`Generated ${snap.result?.created ?? 0} new test cases`);
      else if (snap.state === 'failed') toast.error(snap.error || 'Generation failed');
  ...
  async function onSave(id, payload) {
    ...
    setActive(updated);
    setData((d) => d && { ...d, items: d.items.map((t) => (t.id === id ? updated : t)) });
  ...
  async function onBulkDelete() {
    if (!window.confirm(`Delete ${selected.length} selected test case(s)?`)) return;
    try {
  ...
  const generateBtn = canWrite && (
    <Button icon={Sparkles} onClick={() => setGenOpen(true)} disabled={!!jobId && !snapshotFinished(snapshot)}>
      Generate test cases
    </Button>
  );
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-sm text-muted">Generated from your requirements. Review, edit and keep what you need.</p>
        </div>
      ...
      {jobId && <GenerationBanner snapshot={snapshot} onDismiss={() => setJobId(null)} />}
      ...
      {loading && !data ? (
        ...
        <Card className="space-y-4">
          <Tabs tabs={tabs} value={type} onChange={(v) => updateFilter('type', v)} />
          ...
          <div className="flex flex-col gap-3 lg:flex-row">
            ...
            <Select
              className="h-10"
              options={[{ value: '', label: 'All feature areas' }, ...(data?.modules ?? []).map((m) => ({ value: m, label: m }))]}
              value={moduleFilter}
          ...
          {selected.length > 0 && canWrite && (
            <div className="flex items-center justify-between gap-2 rounded-xl bg-primary-soft px-3 py-2 text-sm">
              <span className="font-medium text-brand">{selected.length} selected</span>
          ...
          {items.length === 0 ? (
            <p className="py-10 text-center text-sm text-muted">
              {hasFilters ? 'No test cases match these filters.' : 'Nothing on this page.'}
            </p>
          ) : (
            <TestCaseTable
              items={items}
