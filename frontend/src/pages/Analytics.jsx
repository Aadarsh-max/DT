...
export default function Analytics() {
  ...
  const deltaText =
    k?.delta != null
      ? `${k.delta >= 0 ? "+" : ""}${k.delta} pts vs previous run`
      : undefined;
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-sm text-muted">
            Pass rate trends, bug severity and feature area health across your
            runs.
          </p>
        </div>
        ...
        <>
          <div className="grid grid-cols-1 gap-3 min-[480px]:grid-cols-2 xl:grid-cols-4">
            ...
            <StatCard
              ...
              value={pct(k.avgPassRate)}
              note="across all runs shown"
            />
            <StatCard
              icon={PlayCircle}
              tone="info"
              ...
              value={k.runs}
              note={
                k.avgDurationSec != null
                  ? `avg ${formatDuration(k.avgDurationSec)}`
                  : undefined
              }
            />
            <StatCard
              ...
              label="Open bugs"
              value={k.openBugs}
              note={`${k.totalBugs} found in this period`}
            />
          </div>

          <DeployRiskCard runId={data.trend.at(-1)?.runId} />
          <Card>
            <CardHeader>
              <CardTitle>Pass rate per run</CardTitle>
              {data.range.runs >= data.range.maxRuns && (
                <Badge>Latest {data.range.maxRuns} runs</Badge>
              )}
            </CardHeader>
            <TrendChart points={data.trend} />
            <p className="mt-2 text-xs text-muted">
              Pass rate = passed ÷ (passed + failed + error). Skipped tests are
              excluded. Hover a point for details.
            </p>
          </Card>
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-6 xl:grid-cols-2">
            <Card>
              ...
              <SeverityChart
                counts={data.bugs.bySeverity}
                unanalyzed={data.bugs.unanalyzed}
              />
            </Card>
            <Card>
              <CardHeader>
              ...
              <ModuleHealthChart
                rows={data.byType.map((t) => ({
                  ...t,
                  name: TEST_TYPE_LABEL[t.name] ?? t.name,
                }))}
              />
            </Card>
          </div>
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted">
                          failed or errored in {t.failures} of {t.runs} run
                          {t.runs === 1 ? "" : "s"}
                        </p>
                      </div>
                      <Badge tone={TEST_TYPE_TONE[t.type]}>
                        {TEST_TYPE_LABEL[t.type]}
                      </Badge>
                    </li>
                  ))}
                </ul>
