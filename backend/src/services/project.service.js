...
const countSelect = {
  _count: { select: { requirements: true, testCases: true, runs: true, bugs: true } },
};
const serialize = ({ _count, ...project }) => ({ ...project, counts: _count });

// Read access: admin, owner or member. manage: admin or owner only.
export async function assertProjectAccess(projectId, user, { manage = false } = {}) {
...
export async function getProject(id, user) {
  ...
  const project = await prisma.project.findUnique({ where: { id }, include: countSelect });
...
export async function updateProject(id, user, data) {
  ...
  const project = await prisma.project.update({ where: { id }, data, include: countSelect });
  return serialize(project);
}
...
export async function deleteProject(id, user) {
  ...
  // Chat messages use SetNull on their project, so they are removed explicitly
  await prisma.chatMessage.deleteMany({ where: { projectId: id } });
  await prisma.project.delete({ where: { id } }); // cascades to all related rows

  await fs.rm(path.join(UPLOAD_DIR, 'screenshots', id), { recursive: true, force: true }).catch(() => {});
  await fs.rm(path.join(UPLOAD_DIR, 'reports', id), { recursive: true, force: true }).catch(() => {});
  await fs.rm(path.join(UPLOAD_DIR, 'apks', id), { recursive: true, force: true }).catch(() => {});

  await aiEngine
    .deleteProject(id)
    .catch((e) => logger.warn(`Could not clear vector index for ${id}: ${e.message}`));
  await Promise.all(
    requirements.filter((r) => r.filePath).map((r) => fs.unlink(r.filePath).catch(() => {}))
  );
}