...
import { REPORT_DIR, collectReportFacts } from '../../services/report.service.js';
  ...
  try {
    ...
    await prisma.report.update({
      ...
      data: {
        ...
        errorMsg: null,
        filePath: rel,
        summary: out.sections.executiveSummary.slice(0, 300),
        content: {
          facts,
    ...
    await prisma.report
      .update({ where: { id: reportId }, data: { status: 'FAILED', errorMsg: String(e.message).slice(0, 500) } })
...
export function startGenerateReportWorker() {
  ...
  worker.on('failed', (job, err) => logger.error(`Report ${job?.id} failed: ${err.message}`));
  worker.on('error', (err) => logger.error(`Report worker error: ${err.message}`));
